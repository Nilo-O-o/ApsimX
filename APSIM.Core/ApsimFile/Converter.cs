...
internal class Converter
{
    ...
    /// <summary>
    /// Update OnEvent function to use events as well as stages
    /// rename SetEvent and ReSetEvent to SetStage and ResetStage because that is what they are
    /// Add set event with cutting and event to respond to to maintain current functionality.
    /// Update <sup></sup> and <sub></sub> tags to use ^^ and ~~ syntax
    /// Replace a hrefs with [text](url) syntax
    /// <param name="root">Root json object.</param>
    /// <param name="_">Unused filename.</param>
    ...
    {
        foreach (var OnEventFunction in JsonUtilities.ChildrenOfType(root, "OnEventFunction"))
        {
            //Collect the existing setting for event to set and reset properties
            string setStage = OnEventFunction["SetEvent"].ToString();
            string reSetStage = OnEventFunction["ReSetEvent"].ToString();

            //Add new properties, SetStage and ResetStage with the same values that were assigned to events
            OnEventFunction["SetStage"] = setStage;
            OnEventFunction["ReSetStage"] = reSetStage;

            //Assign blank values to SetEvent.  
            OnEventFunction["SetEvent"] = "";
            //OnEventFunction did have a hardcoded reset on cutting event.  This has been deleted and moved into reset event
            //to allow more flexibility in functionsetup
            OnEventFunction["ReSetEvent"] = "[BiomassRemovalEvents].Cutting";
        }
    }
}
